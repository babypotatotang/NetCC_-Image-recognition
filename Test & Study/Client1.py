...
resolution = (480, 854)
img = r'D:\git repos\NetCC\NetCC_-Image-recognition\python\20201107_1_multiple.mp4'
index=1
...
# Perspective Point
point_list = []
point_list.append((218, 376))  # Left Up
point_list.append((296, 377))  # Right Up
point_list.append((151, 768))  # Left Down
point_list.append((318, 778))  # Right Down
...
def webcam(queue):
    ...
    while True:
        ...
        if (ret is True) and (current_time > 1. / FPS) and (cnt == int(wait)):
            ...
            now = time.localtime()
            now_s='[1] : D'+str(now.tm_mday)+' H'+str(now.tm_hour)+' M'+str(now.tm_min)+' S'+str(now.tm_sec)
            print(now_s)
            #sock.send(now_s.encode('utf-8'))

            process = detect(gray, frame)

            cv2.imshow('test1', process)
            # end = time.time()
            # diff = end-start
            # print(diff)
